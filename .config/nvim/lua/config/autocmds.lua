...
-- Function to find a 'package.json' by searching up the directory tree.
local function find_npm_root(start_dir)
  local current_dir = start_dir
  -- A loop to walk up the directory tree until we hit the root.
  while current_dir ~= "/" do
    -- Check for the existence of a 'package.json' file.
  ...
  return nil -- Return nil if 'package.json' is not found.
end
...
vim.api.nvim_create_autocmd("BufEnter", {
  ...
  callback = function()
    ...
    -- Step 1: Try to find a 'package.json' as the project root.
    local npm_root = find_npm_root(file_dir)
    if npm_root then
      target_dir = npm_root
    else
      -- Step 2: If no 'package.json' is found, fall back to 'git rev-parse'.

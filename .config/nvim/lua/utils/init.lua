...
helpers.install = helpers_installation or { home = stdpath("config") }
--- external helpers configuration folder
helpers.install.config = stdpath("config"):gsub("nvim$", "helpers")
vim.opt.rtp:append(helpers.install.config)
--- supported helpers user conifg folders
helpers.supported_configs = { helpers.install.home, helpers.install.config }
...
local function load_module_file(module)
  ...
  for _, config_path in ipairs(helpers.supported_configs) do
...
-- @function helpers.func_or_extend
...
local function func_or_extend(overrides, default, extend)
  ...
  if extend then
    ...
    if type(overrides) == "table" then
      default = helpers.extend_tbl(default, overrides)
...
function helpers.extend_tbl(default, opts)
  opts = opts or {}
...
function helpers.conditional_func(func, condition, ...)
...
function helpers.get_hlgroup(name, fallback)
...
function helpers.trim_or_nil(str)
...
function helpers.pad_string(str, padding)
  padding = padding or {}
...
function helpers.initialize_icons()
  helpers.icons = helpers.user_opts("icons", require("core.icons.nerd_font"))
  helpers.text_icons = helpers.user_opts("text_icons", require("core.icons.text"))
end

--- Get an icon from `lspkind` if it is available and return it
-- @param kind the kind of icon in `lspkind` to retrieve
-- @return the icon
function helpers.get_icon(kind)
  local icon_pack = vim.g.icons_enabled and "icons" or "text_icons"
  if not helpers[icon_pack] then
    helpers.initialize_icons()
  end
  return helpers[icon_pack] and helpers[icon_pack][kind] or ""
end

--- Serve a notification with a title of RylaNvim
...
function helpers.notify(msg, type, opts)
  vim.schedule(function()
    vim.notify(msg, type, helpers.extend_tbl({ title = "RylaNvim" }, opts))
  end)
end

--- Trigger an RylaNvim user event
-- @param event the event name to be appended to Astro
function helpers.event(event)
  vim.schedule(function()
...
function helpers.add_buffer_autocmd(augroup, bufnr, autocmds)
...
function helpers.echo(messages)
...
function helpers.confirm_prompt(messages)
  if messages then
    helpers.echo(messages)
  end
...
local function user_setting_table(module)
  ...
  local settings = helpers.user_settings or {}
...
function helpers.vim_opts(options)
...
function helpers.user_opts(module, default, extend)
...
function helpers.system_open(path)
  local cmd
  ...
  if not cmd then
    helpers.notify("Available system opening tool not found!", "error")
  end
...
function helpers.toggle_term_cmd(opts)
  local terms = helpers.user_terminals
  ...
  -- toggle the terminal
  helpers.user_terminals[opts.cmd][num]:toggle()
end
...
function helpers.null_ls_providers(filetype)
  local registered = {}
  -- try to load null-ls
...
function helpers.null_ls_sources(filetype, method)
  local methods_avail, methods = pcall(require, "null-ls.methods")
  return methods_avail and helpers.null_ls_providers(filetype)[methods.internal[method]] or {}
end
...
function helpers.alpha_button(sc, txt)
...
function helpers.load_plugin_with_func(plugin, module, func_names)
...
function helpers.set_mappings(map_table, base)
...
function helpers.delete_url_match()
...
function helpers.set_url_match()
  helpers.delete_url_match()
  if vim.g.highlighturl_enabled then
    vim.fn.matchadd("HighlightURL", helpers.url_matcher, 15)
  end
end
...
function helpers.cmd(cmd, show_error)
...
function helpers.is_valid_buffer(bufnr)
...
function helpers.close_buf(bufnr, force)
  if force == nil then
    force = false
  end
  local current = vim.api.nvim_get_current_buf()
  ...
  if bufnr == current then
    helpers.nav_buf(vim.t.bufs[1] == current and 1 or -1)
  end

  if helpers.is_available("bufdelete.nvim") then

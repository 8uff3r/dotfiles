...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    default=["$HOME/.config/hypr/custom/keybinds.conf"],
    help="path(s) to keybind file(s) (sourcing isn't supported). Multiple paths will be concatenated in order.",
)
...
def get_unbind_key_at_line(line_number, line_start=0):
    ...
    (
        mods,
        key,
    ) = list(map(str.strip, keys.split(",", 2)))
    if mods:
        modstring = (
...
def remove_keybind_recursive(section, target_key, target_mods):
    ...
    for child in section["children"]:
        remove_keybind_recursive(child, target_key, target_mods)
    # Remove empty children
    for childn in section["children"]:
        if len(childn["keybinds"]) == 0 and len(childn["children"]) == 0:
            section["children"].remove(childn)
    ...
    while reading_line < len(content_lines):  # TODO: Adjust condition
        ...
        if (heading_search_result != None) and (
            ...
            current_content["children"].append(
                get_binds_recursive(
                    Section([], [], section_name), heading_scope, root_section
                )
            )
            ...
            not line.lstrip().startswith("bind")
            and not line.lstrip().startswith("unbind")
        ):  # Comment, ignore
            pass
